...
var newVoice = tj.configuration.speak.voice;
...
// my own see function to pass a file path to the function
function mySee(filePath) {
  tj._assertCapability('see');

  return new Promise(function(resolve, reject) {
    // console.log("TJBot taking a photo");
    tj.takePhoto(filePath).then(function(filePath) {
      resolve(tj.recognizeObjectsInPhoto(filePath))
    });
  });
}
...
function takePic() {
  ...
  mySee(myfilePath).then(function(results) {
...
function translatePhrase(msg) {
  tj.translate(msg, 'en', langTranslateTo).then(function(translation) {
    ...
    var originalVoice = tj.configuration.speak.voice;
    ...
    tj.configuration.speak.voice = newVoice;
    ...
    tj.configuration.speak.voice = originalVoice;
